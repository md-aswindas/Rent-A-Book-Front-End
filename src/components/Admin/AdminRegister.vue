<template>
  <div class="container" :class="{ 'dark-mode': isDarkMode }">
    ...
    <div class="form-section">
      <h2 class="main-text">Admin Registration</h2>
      ...
      <input 
        ...
        :class="{ 'error': errors.email }"
         @input="validateField('email'); validatePreviousFields('email')"
      >
      ...
      <div class="password-container">
        <input 
          ...
          @input="validateField('password'); validatePreviousFields('password')"
        >
        <button 
          type="button" 
          class="eye-icon" 
      ...
      <div class="password-container">
        <input 
          ...
          @input="validateField('confirmPassword'); validatePreviousFields('confirmPassword')"
        >
        <button 
          type="button" 
          class="eye-icon" 
        ...
        <div class="user-type-row">
          ...
          <div class="toggle-switch">
            <div class="toggle-options">
              ...
              <span 
                class="toggle-option" 
                :class="{ 'active': form.userType === 'artist' }"
                @click="form.userType = 'artist'"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <button class="text-field btn" @click="register">Create Account</button>
      ...
      <p class="signup">Already have an account? <router-link to="/AdminLogin" class="login-link"><span class="login-link">Log in</span></router-link></p>
    </div>
...
export default {
  ...
  methods: {
      ...
      switch (field) {
        ...
        case 'userType':
          if (!this.form.userType) {
            this.errors.userType = 'Please select account type';
          } else {
    ...
    validatePreviousFields(currentField) {
      const order = ['name', 'email', 'password', 'confirmPassword', 'userType'];
      const index = order.indexOf(currentField);
      if (index > 0) {
        for (let i = 0; i < index; i++) {
          this.validateField(order[i]);
        }
      }
    },
    register() {
      const fields = ['name', 'email', 'password', 'confirmPassword', 'userType'];
      this.errors = {};
...
.container.dark-mode .text-field.error {
  border-color: #ff6b6b;
}
...
.container.dark-mode .user-type-label {
  color: #e0e0e0;
}
...
.container.dark-mode .toggle-options {
  background-color: #3d3d3d;
}

.container.dark-mode .toggle-option {
  color: #999;
}

.container.dark-mode .toggle-option:hover {
  background-color: #4d4d4d;
}

.container.dark-mode .toggle-option.active {
  background-color: #e0e0e0;
  color: #333;
}
